...
**PubSub Applications** exchange messages formatted as the `NetworkMessage` structure using underlying communication stack. As illustrated in the following domain model, directly or indirectly the specification defines the following actors:

- `Publisher`: pushes the current process data formatted as the `NetworkMessage` structure to an underlying communication stack
- `Subscriber`: consumes the process data, which is recovered from the `NetworkMessages` structure polled from the underlying communication stack
- `Distribution Channel` - select common underlying communication stack
- `NetworkMessage` - the network structure formatted in compliance with the syntax and semantics defined by the [OPC.UA.PubSub][OPC.UA.PubSub] specification
...
The section [Walk-through `ReferenceApplication`][Walk-through `ReferenceApplication`] covers description of a project aimed at implementation of an example of the `OOI Reactive Application` supporting producer and consumer roles simultaneously. The purpose of the `ReferenceApplication` is to demonstrate the concepts and architecture of the reactive networking application implementation, rather than to necessarily provide a realistic scenario for its use.

## Acknowledgment

I would like to thank ..... for their feedback, cooperation and of course friendship.
...
- [Walk-through `ReferenceApplication`][Walk-through `ReferenceApplication`]
