...
namespace UAOOI.Networking.UDPMessageHandler.UnitTest
{
  ...
  public class BinaryUDPPackageReaderTestClass
  {
    ...
    public void CreatorTestMethod()
    {
      ...
      using (BinaryUDPPackageReader _reader1 = new BinaryUDPPackageReader(new UADecoder(), 4840, x => Console.WriteLine(x)))
      {
    ...
    public void ExclusiveAddressUseTrueTestMethod()
    {
      ...
      using (BinaryUDPPackageReader _reader1 = new BinaryUDPPackageReader(new UADecoder(), 4840, x => Console.WriteLine(x)))
      {
        ...
        _reader1.ReuseAddress = _ExclusiveAddressUse;
        _reader1.State.Enable();
        using (BinaryUDPPackageReader _reader2 = new BinaryUDPPackageReader(new UADecoder(), 4840, x => Console.WriteLine(x)))
        {
    ...
    public void ExclusiveMulticastGroupTestMethod()
    {
      ...
      using (BinaryUDPPackageReader _reader1 = new BinaryUDPPackageReader(new UADecoder(), 4840, x => _traceList.Add(x)))
      {
        try
        {
          Assert.IsNotNull(_reader1);
